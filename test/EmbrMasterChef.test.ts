...
describe("EmbrMasterChef", function () {
  ...
  it("sets initial state correctly", async () => {
    ...
    const embrPerSec = bn(6)

    const chef = await deployChef(embr.address, treasury.address, embrPerSec, startBlock)
    ...
    const actualEmbrPerBlock = await chef.embrPerSec()
    ...
    expect(actualEmbrPerBlock).to.equal(embrPerSec)
  })
  ...
  it("allows setting of emission rate by the owner", async () => {
    ...
    expect(await chef.embrPerSec()).to.equal(bn(1))
  })
  ...
  it("starts giving out rewards only after the start block has been reached", async function () {
    const embrPerSec = bn(6)
    const chef = await deployChef(embr.address, treasury.address, embrPerSec, 150)
    await embr.transferOwnership(chef.address)
    ...
    expect(await embr.balanceOf(bob.address)).to.equal(percentageOf(embrPerSec, lpPercentage))
    expect(await embr.balanceOf(treasury.address)).to.equal(percentageOf(embrPerSec, treasuryPercentage))
    expect(await embr.totalSupply()).to.equal(embrPerSec)

    await advanceBlockTo("154")

    await chef.connect(bob).harvest(0, bob.address) // block 105
    expect(await embr.balanceOf(bob.address)).to.equal(percentageOf(embrPerSec.mul(5), lpPercentage))
    expect(await embr.balanceOf(treasury.address)).to.equal(percentageOf(embrPerSec.mul(5), treasuryPercentage))
    expect(await embr.totalSupply()).to.equal(embrPerSec.mul(5))
  })
  ...
  it("distributes EMBR properly for each staker", async function () {
     ...
     */
    const embrPerSec = bn(6)
    const chef = await deployChef(embr.address, treasury.address, embrPerSec, 300)
    await embr.transferOwnership(chef.address)

    const lpRewards = rewardsCalculator(embrPerSec, lpPercentage)
    const treasuryRewards = rewardsCalculator(embrPerSec, treasuryPercentage)
   ...
   */
    ...
    expect(await embr.totalSupply()).to.equal(embrPerSec.mul(10))
    ...
    expect(await embr.balanceOf(chef.address)).to.equal(embrPerSec.mul(10).sub(aliceBalance320).sub(bobBalance316).sub(treasuryBalance))
   ...
   */
    ...
    expect(await embr.totalSupply()).to.equal(embrPerSec.mul(20))
    ...
    const treasuryBalance330 = percentageOf(embrPerSec.mul(20), treasuryPercentage)
    expect(await embr.balanceOf(chef.address)).to.equal(embrPerSec.mul(20).sub(aliceBalance320).sub(bobBalance330).sub(treasuryBalance330))
    ...
    */
    ...
    expect(await embr.totalSupply()).to.equal(embrPerSec.mul(50))
  ...
  it("gives correct EMBR allocation to each pool", async function () {
    const embrPerSec = bn(6)
    const chef = await deployChef(embr.address, treasury.address, embrPerSec, 100)
    await embr.transferOwnership(chef.address)
...
function rewardsCalculator(embrPerSec: BigNumber, percentage: number) {
  return (blocks: number) => {
    return percentageOf(embrPerSec.mul(blocks), percentage)
  }
}
